...
export default {
    ...
    likePost: async ({ dispatch }, { post_id }) => {
        await axios
            .post("/like/", { post_id: post_id }).then(() => {
                dispatch('getPosts')
            })
    },
    unlikePost: async ({ dispatch }, { post_id }) => {
        await axios
            .post("/unlike/", { post_id: post_id }).then(() => {
                dispatch('getPosts')
            })
    },
    likeComment: async ({ dispatch }, { comment_id }) => {
        await axios
            .post("/like-comment/", { comment_id: comment_id }).then(() => {
                dispatch('getPosts')
            })
    },
    unlikeComment: async ({ dispatch }, { comment_id }) => {
        await axios
            .post("/unlike-comment/", { comment_id: comment_id }).then(() => {
                dispatch('getPosts')
            })
    },
    removePost: async ({ dispatch }, { post_id }) => {
        await axios
            .post("/delete-post/", { post_id: post_id }).then(() => {
                dispatch('getPosts')
            })
    },
    removeComment: async ({ dispatch }, { comment_id }) => {
        await axios
            .post("/remove-comment/", { comment_id: comment_id }).then(() => {
                dispatch('getPosts')
            })
    },
    removeReply: async ({ dispatch }, { reply_id }) => {
        await axios
            .post("/remove-comment-reply/", { reply_id: reply_id }).then(() => {
                dispatch('getPosts')
            })
    },
}